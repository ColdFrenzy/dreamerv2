...
class TransitionBuffer:
    def __init__(
        ...
        self.observation = {
            agent_id: np.empty((capacity, *obs_shape), dtype=obs_type)
            for agent_id in agents_name
        }
        self.action = {
            agent_id: np.empty((capacity, action_size), dtype=np.float32)
            for agent_id in agents_name
        }
        self.reward = {
            agent_id: np.empty((capacity,), dtype=np.float32)
            for agent_id in agents_name
        }
        self.terminal = {
            agent_id: np.empty((capacity,), dtype=bool) for agent_id in agents_name
        }
    ...
    def add(
        ...
        assert (
            obs.keys() == action.keys() == reward.keys() == done.keys()
        ), "observations, actions, rewards, and dones have not the same keys"
        for agent_id in obs:
...
Episode = namedtuple(
    "Episode", ["observation", "action", "reward", "terminal", "length"]
)
...
class EpisodicBuffer:
    ...
    def sample(self):
        ...
        obs_batch = np.empty(
            [seq_len, batch_size, *self.obs_shape], dtype=self.obs_type
        )
        act_batch = np.empty(
            [seq_len, batch_size, self.action_size], dtype=self.action_type
        )
        ...
        for ind, episode in enumerate(episode_list):
            (
                ...
                rew_batch[:, ind],
                term_batch[:, ind],
            ) = self._sample_seq(episode, seq_len)
...
class FluidEpisodicBuffer:
    ...
    def sample(self):
        ...
        episode_idx = np.random.choice(
            np.where(np.array(self.lengths) >= seq_len)[0], size=batch_size
        )
        for i, idx in enumerate(episode_idx):
            (
                ...
                rew_batch[:, i],
                term_batch[:, i],
            ) = self._sample_seq(self.buffer[idx], self.lengths[idx], seq_len)
        return obs_batch, act_batch, rew_batch, term_batch
