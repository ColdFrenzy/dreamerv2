...
class Evaluator(object):
    ...
    def load_model(self, config, model_path):
        ...
        if config.pixel:
            self.ObsEncoder = (
                ObsEncoder(obs_shape, embedding_size, config.obs_encoder)
                .to(self.device)
                .eval()
            )
            self.ObsDecoder = (
                ObsDecoder(obs_shape, modelstate_size, config.obs_decoder)
                .to(self.device)
                .eval()
            )
        else:
            ...
            self.ObsDecoder = (
                DenseModel(obs_shape, modelstate_size, config.obs_decoder)
                .to(self.device)
                .eval()
            )
